...
function slug(value: unknown): string {
  return String(value ?? '').trim().toLowerCase().replace(/&/g, 'and').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || `item-${Date.now()}`;
}
...
async function ensureAdminTables() {
  await query(`create table if not exists insurance_agencies (id bigserial primary key, agency_key text not null unique, agency_name text not null, legal_name text, display_name text, brand_name text, service_area text, lines_of_business text[] not null default '{}', status text not null default 'ACTIVE', notes text, created_at timestamptz not null default now(), updated_at timestamptz not null default now())`);
  await query(`create table if not exists insurance_agents (id bigserial primary key, agent_key text not null unique, agency_key text, agent_name text not null, email text, phone text, role text not null default 'AGENT', status text not null default 'ACTIVE', lines_of_business text[] not null default '{}', notes text, created_at timestamptz not null default now(), updated_at timestamptz not null default now())`);
  await query(`insert into insurance_agencies (agency_key, agency_name, legal_name, display_name, brand_name, service_area, lines_of_business, status, notes) values ('invicta-capital-group', 'Invicta Capital Group', 'Invicta Capital Group', 'Invicta Capital Group', 'Invicta', 'Nationwide', array['Property & Casualty','Life','Health']::text[], 'ACTIVE', 'Default agency for Invicta commercial insurance lead engine.') on conflict (agency_key) do update set agency_name = excluded.agency_name, updated_at = now()`);
}

async function listAgencies() {
  await ensureAdminTables();
  const result = await query(`select a.*, count(ag.id)::int as agent_count from insurance_agencies a left join insurance_agents ag on ag.agency_key = a.agency_key and ag.status = 'ACTIVE' group by a.id order by a.created_at desc`);
  return result.rows;
}

async function listAgents() {
  await ensureAdminTables();
  const result = await query(`select ag.*, a.agency_name from insurance_agents ag left join insurance_agencies a on a.agency_key = ag.agency_key order by ag.created_at desc`);
  return result.rows;
}
...
function adminPageHtml(): string {
  return `<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Invicta Admin</title><style>body{margin:0;background:#0b1220;color:#e2e8f0;font-family:Arial,sans-serif}.top{border-bottom:1px solid #1e293b;padding:14px 24px;display:flex;justify-content:space-between}.links{display:flex;gap:16px;color:#94a3b8;font-size:13px;flex-wrap:wrap}.wrap{max-width:1200px;margin:0 auto;padding:30px 22px}.card{background:#111a2b;border:1px solid #1e293b;border-radius:12px;padding:18px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:14px;margin:18px 0}.two{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-top:18px}.num{font-size:34px;font-weight:800}.muted{color:#64748b;font-size:13px}.label{color:#cbd5e1;font-size:13px;font-weight:700;margin-top:12px}button{border:0;border-radius:8px;padding:9px 13px;background:#2563eb;color:white;font-weight:700;cursor:pointer}button.gray{background:#1e293b;color:#94a3b8;border:1px solid #283548}button.red{background:#7f1d1d;color:#fecaca}input,select{background:#16202f;border:1px solid #283548;border-radius:8px;padding:9px 11px;color:#e2e8f0}input{width:100%}.form3{display:grid;grid-template-columns:1fr 1fr auto;gap:10px}.form2{display:grid;grid-template-columns:1fr auto;gap:10px;margin-top:10px}table{width:100%;border-collapse:collapse;font-size:13px;margin-top:12px}td,th{border-bottom:1px solid #16202f;padding:10px;text-align:left}th{color:#64748b;text-transform:uppercase;font-size:11px}.hidden{display:none}.ok{color:#4ade80}.bad{color:#f87171}pre{background:#07111f;border:1px solid #1e293b;border-radius:12px;padding:12px;white-space:pre-wrap;max-height:340px;overflow:auto}@media(max-width:850px){.two,.form3{grid-template-columns:1fr}.top{display:block}}</style></head><body><div class="top"><b>Invicta Admin</b><div class="links"><span>Overview</span><span>Agencies</span><span>Agents</span><span>API Pulls</span><span>Integrations</span><span id="sessionPill">Signed out</span></div></div><main class="wrap"><section id="loginCard" class="card" style="max-width:520px;margin:60px auto"><h1>Admin Login</h1><p class="muted">Enter the admin key.</p><input id="adminKey" type="password" placeholder="Admin key"/><div style="display:flex;gap:10px;margin-top:12px"><button id="loginBtn">Open admin</button><button class="gray" id="clearBtn">Clear</button></div><p id="loginStatus" class="muted"></p></section><section id="dashboard" class="hidden"><div style="display:flex;justify-content:space-between;gap:16px"><div><h1>Overview</h1><p class="muted">Manage agencies, agents, API pulls, integrations, and admin operations.</p></div><button class="red" id="logoutBtn">Logout</button></div><div class="grid"><div class="card"><div class="num" id="agencyCount">-</div><div class="label">Active agencies</div></div><div class="card"><div class="num" id="agentCount">-</div><div class="label">Active agents</div></div><div class="card"><div class="num" id="apiPullCount">-</div><div class="label">API / data pulls</div></div><div class="card"><div class="num" id="readyCount">-</div><div class="label">Ready leads</div></div></div><div class="two"><section class="card"><h2>Agencies</h2><p class="muted">Add agencies and review active agent counts.</p><div class="form3"><input id="agencyNameInput" placeholder="Agency name"/><input id="agencyServiceInput" value="Nationwide"/><button id="addAgencyBtn">+ New agency</button></div><table><thead><tr><th>Agency</th><th>Key</th><th>Agents</th><th>Status</th></tr></thead><tbody id="agencyRows"></tbody></table></section><section class="card"><h2>Agents</h2><p class="muted">Add agents under an agency.</p><div class="form3"><input id="agentNameInput" placeholder="Agent name"/><input id="agentEmailInput" placeholder="Email"/><select id="agentAgencySelect"></select></div><div class="form2"><input id="agentPhoneInput" placeholder="Phone"/><button id="addAgentBtn">+ New agent</button></div><table><thead><tr><th>Agent</th><th>Email</th><th>Agency</th><th>Status</th></tr></thead><tbody id="agentRows"></tbody></table></section></div><div class="two"><section class="card"><h2>Recent API / data pulls</h2><button class="gray" id="refreshBtn">Refresh</button><table><thead><tr><th>Source</th><th>Details</th><th>Date</th></tr></thead><tbody id="pullRows"></tbody></table></section><section class="card"><h2>Integration status</h2><table><tbody id="integrationRows"></tbody></table></section></div><div class="two"><section class="card"><h2>Administrator operations</h2><div style="display:grid;gap:10px"><button class="gray" id="dbInitBtn">Initialize DB schema</button><button id="importBtn">Import 1,000 carriers</button><button id="scoreBtn">Refresh scores</button><div style="display:grid;grid-template-columns:1fr 90px;gap:8px"><button id="txBtn">Run Texas verification</button><select id="txLimit"><option>10</option><option>25</option><option>50</option></select></div><div style="display:grid;grid-template-columns:1fr 90px;gap:8px"><button id="flBtn">Run Florida verification</button><select id="flLimit"><option>10</option><option>25</option><option>50</option></select></div><button class="gray" id="arkonBtn">Test CRM export: 1 lead</button></div></section><section class="card"><h2>Audit output</h2><pre id="output">Ready.</pre></section></div></section></main><script>const loginCard=document.getElementById('loginCard'),dashboard=document.getElementById('dashboard'),output=document.getElementById('output');function k(){return sessionStorage.getItem('FMCSA_ADMIN_KEY')||''}function h(){return{'content-type':'application/json','x-admin-api-key':k()}}function f(v){return v===null||v===undefined||v===''?'N/A':String(v)}function yn(v){return v?'<span class="ok">Configured</span>':'<span class="bad">Not configured</span>'}function out(v){output.textContent=typeof v==='string'?v:JSON.stringify(v,null,2)}async function api(p,o){const r=await fetch(p,o||{}),t=await r.text();let d;try{d=JSON.parse(t)}catch(e){d={raw:t}}if(!r.ok)throw new Error(d.error||t);return d}async function get(p){return api(p,{headers:h()})}async function post(p,b){return api(p,{method:'POST',headers:h(),body:JSON.stringify(b||{})})}function show(){loginCard.classList.add('hidden');dashboard.classList.remove('hidden');sessionPill.textContent='Signed in'}function hide(){dashboard.classList.add('hidden');loginCard.classList.remove('hidden');sessionPill.textContent='Signed out'}async function load(){const d=await get('/admin/overview'),m=d.metrics;agencyCount.textContent=f(m.agencies.active);agentCount.textContent=f(m.agents.active);apiPullCount.textContent=f(m.apiPulls.total);readyCount.textContent=f(m.leads.salesReady);integrationRows.innerHTML='<tr><td>Admin auth</td><td>'+yn(d.integrations.adminApiKeyConfigured)+'</td></tr><tr><td>CRM webhook</td><td>'+yn(d.integrations.arkonWebhookConfigured)+'</td></tr><tr><td>Sheets webhook</td><td>'+yn(d.integrations.googleSheetsWebhookConfigured)+'</td></tr><tr><td>Texas API key</td><td>'+yn(d.integrations.txComptrollerKeyConfigured)+'</td></tr>';pullRows.innerHTML=(d.recentPulls||[]).length?d.recentPulls.map(x=>'<tr><td>'+f(x.source)+'</td><td>'+f(x.details)+'</td><td>'+f(x.date)+'</td></tr>').join(''):'<tr><td colspan="3">No pull history yet.</td></tr>';agencyRows.innerHTML=(d.agencies||[]).map(a=>'<tr><td>'+f(a.agency_name)+'</td><td>'+f(a.agency_key)+'</td><td>'+f(a.agent_count)+'</td><td>'+f(a.status)+'</td></tr>').join('');agentRows.innerHTML=(d.agents||[]).length?d.agents.map(a=>'<tr><td>'+f(a.agent_name)+'</td><td>'+f(a.email)+'</td><td>'+f(a.agency_name||a.agency_key)+'</td><td>'+f(a.status)+'</td></tr>').join(''):'<tr><td colspan="4">No agents yet.</td></tr>';agentAgencySelect.innerHTML=(d.agencies||[]).map(a=>'<option value="'+f(a.agency_key)+'">'+f(a.agency_name)+'</option>').join('')}async function login(){sessionStorage.setItem('FMCSA_ADMIN_KEY',adminKey.value.trim());try{await get('/admin/overview');show();await load()}catch(e){sessionStorage.removeItem('FMCSA_ADMIN_KEY');loginStatus.textContent=e.message}}async function run(label,fn){out(label+'...');try{const d=await fn();out(d);await load()}catch(e){out({ok:false,error:e.message})}}loginBtn.onclick=login;clearBtn.onclick=()=>{adminKey.value='';sessionStorage.removeItem('FMCSA_ADMIN_KEY')};logoutBtn.onclick=()=>{sessionStorage.removeItem('FMCSA_ADMIN_KEY');hide()};refreshBtn.onclick=()=>run('Refreshing admin overview',load);dbInitBtn.onclick=()=>run('Initializing schema',()=>post('/admin/db/init',{}));importBtn.onclick=()=>run('Importing carriers',()=>post('/admin/import',{source:'company-census',limit:1000}));scoreBtn.onclick=()=>run('Refreshing scores',()=>post('/admin/score/refresh',{}));txBtn.onclick=()=>run('Running Texas verification',()=>post('/admin/enrich/texas',{limit:Number(txLimit.value)}));flBtn.onclick=()=>run('Running Florida verification',()=>post('/admin/enrich/fl',{limit:Number(flLimit.value)}));arkonBtn.onclick=()=>run('Testing CRM export',()=>post('/admin/export/arkon',{limit:1,minGrade:'B'}));addAgencyBtn.onclick=()=>run('Creating agency',()=>post('/admin/agencies',{agencyName:agencyNameInput.value,serviceArea:agencyServiceInput.value}));addAgentBtn.onclick=()=>run('Creating agent',()=>post('/admin/agents',{agentName:agentNameInput.value,email:agentEmailInput.value,phone:agentPhoneInput.value,agencyKey:agentAgencySelect.value}));if(k()){show();load().catch(e=>out({ok:false,error:e.message}))}</script></body></html>`;
}
